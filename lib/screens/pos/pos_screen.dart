...
class PosScreenState extends ConsumerState<PosScreen> {
  ...
  void _showAddToCartDialog(Product product) {
    showDialog(
      context: context,
      builder: (context) => AddToPosCartDialog(product: product),
    );
  }
            ...
            children: [
              ...
              Column(
                ...
                children: [
                  ...
                  if (product.price > 0)
                    Text(currencyFormatter.format(product.price), style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Colors.green)),
                ],
              ),
            ],
          ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [const Text('Total', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), Text(currencyFormatter.format(total), style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.green))],
            ),
    ...
    return ListTile(
      ...
      trailing: Row(
        ...
        children: [
          ...
          IconButton(icon: const Icon(Icons.delete, size: 18, color: Colors.redAccent), onPressed: () => ref.read(posCartProvider.notifier).removeItem(item.product.id)),
        ],
      ),
    );
  }
